...
test('refresh IDs reuse fixed collectors, persist admission and replace only explicit matching baseline sources',async t=>{
  ...
  const host=createHomelabMcpServer({schemaVersion:1,provider:'copilot',runsDirectory:join(root,'jobs'),observations:[baseline],targets:[],refreshSources:[source],maxJobs:4},
...
test('refresh configuration and result provenance cannot substitute hosts, commands or raw fields',()=>{
  ...
  assert.throws(()=>HomelabMcpConfig.parse({schemaVersion:1,provider:'copilot',runsDirectory:'x',observations:[],targets:[],refreshSources:[source,source]}));
