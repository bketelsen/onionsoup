...
export function findingCounts(raw:unknown){
  const {findings}=TriageResult.parse(raw);
  return {attentionNow:findings.filter(f=>f.classification==='attention_now').length,historical:findings.filter(f=>f.classification==='historical').length,
    insufficientEvidence:findings.filter(f=>f.classification==='insufficient_evidence').length,selected:findings.length};
}
...
export function classificationEvidence(input:WorkloadObservation,podId:string,at:string){
  ...
  const future=chain.some(f=>[f.createdAt,f.deletingAt,...(f.kind==='Workflow'?[f.finishedAt]:[]),...(f.kind==='Pod'?f.containers.flatMap(c=>[c.finishedAt,c.lastFinishedAt]):[])].some(atTime=>atTime!==null&&Date.parse(atTime)>Date.parse(at)));
  ...
  const workflow=chain.find(f=>f.kind==='Workflow');
  const workflowTiming=workflow?.kind==='Workflow'&&workflow.finishedAt!==null&&workflow.createdAt!==null&&Date.parse(workflow.finishedAt)>=Date.parse(workflow.createdAt);
  const recovered=chain.slice(1).find(f=>f.kind==='Job'?f.complete==='True'&&f.failed!=='True'&&!f.deletingAt:f.kind==='Workflow'?f.phase==='Succeeded'&&workflowTiming&&!f.deletingAt:healthy(f));
  const historical=Boolean(recovered)&&!errorWaiting&&(pod.phase==='Failed'||pod.phase==='Succeeded'||pod.phase==='Running'&&pod.ready==='True');
  const workflowSignal=!workflow||workflow.kind==='Workflow'&&!workflow.deletingAt&&pod.phase==='Failed'&&(workflow.phase==='Running'&&!workflow.finishedAt||['Failed','Error'].includes(workflow.phase)&&workflowTiming);
  const attention=workflowSignal&&!historical&&!pod.deletingAt&&(errorWaiting||chain.length>1&&(pod.phase==='Failed'||pod.phase==='Running'&&pod.ready==='False'||pod.phase==='Pending'));
  return {attention,historical,witnesses:recovered?chain.slice(0,chain.indexOf(recovered)+1).map(f=>f.id):[]};
}
class FindingRejection extends Error { constructor(code:string,readonly podId:string){super(code);} }
export function validateTriageResult(raw:unknown,input:WorkloadObservation,at:string):TriageResult{
  ...
  for(const f of result.findings){const chain=ownerChain(input,f.podId).map(r=>r.id);
    if(new Set(f.evidenceIds).size!==f.evidenceIds.length||!f.evidenceIds.includes(f.podId)||f.evidenceIds.some(id=>!chain.includes(id)))throw new FindingRejection('INVALID_EVIDENCE',f.podId);
    const gate=classificationEvidence(input,f.podId,at);
    if(f.classification==='historical'&&(!gate.historical||gate.witnesses.some(id=>!f.evidenceIds.includes(id))))throw new FindingRejection('RECOVERY_NOT_ESTABLISHED',f.podId);
    if(f.classification==='attention_now'&&(!gate.attention||ownerChain(input,f.podId).some(r=>r.kind==='Workflow'&&!f.evidenceIds.includes(r.id))))throw new FindingRejection('CURRENT_SIGNAL_NOT_ESTABLISHED',f.podId);
    if(f.classification!=='historical'&&f.nextInvestigation==='none')throw new FindingRejection('NEXT_INVESTIGATION_REQUIRED',f.podId);
    if(/[<>\u0000-\u0008]|(?:https?:\/\/|BEGIN.*PRIVATE KEY|\bBearer\s)/i.test(f.reason))throw new FindingRejection('UNSAFE_PROSE',f.podId);
  }return result;
}
...
export const TriageRun=z.object({schemaVersion:z.literal(1),kind:z.literal('workload-triage'),runId:z.uuid(),assetId:z.string().regex(/^[a-z][a-z0-9-]{0,63}$/),
  input:WorkloadObservation,inputHash:z.string().regex(/^[a-f0-9]{64}$/),promptVersion:z.enum(['workload-triage-v1','workload-triage-v2','workload-triage-v3','workload-triage-v4']),provider:z.enum(['copilot','codex']),model:z.string().regex(/^[a-zA-Z0-9._-]{1,100}$/),
  ...
  try{if(r.input.schemaVersion===2&&!['workload-triage-v3','workload-triage-v4'].includes(r.promptVersion))throw 0;
    if(digest(r.input)!==r.inputHash||r.assetId!==r.input.assetId)throw 0;
    ...
    try{
      ...
      const submit=defineToolInterface({name:'submit_result',description:'Submit the evidence-linked workload assessment. No service action.',input:TriageResult}).define(async raw=>{
        ...
        try{accepted=validateTriageResult(raw,input,record.startedAt);}catch(error){
          ...
          event('resultRejected',code);await save();throw new Error(JSON.stringify({code,...(error instanceof FindingRejection?{podId:error.podId}:{}),correction:'Check classificationPrerequisites and cite only supplied facts. Use insufficient_evidence when confidence prerequisites are absent.'}));
        }
        return 'Accepted';});
      ...
      const run=agent.run({state:startState([{role:'user',content:JSON.stringify({snapshot:input,assessedAt:record.startedAt,evidenceValidity:evidenceValidity(input,record.startedAt),classificationPrerequisites:input.selected.map(podId=>{const gate=classificationEvidence(input,podId,record.startedAt);return {podId,historicalAllowed:gate.historical,attentionAllowed:gate.attention,requiredHistoricalEvidenceIds:gate.witnesses};})})}]),signal,stream:true});
...
export function workloadCapabilityManifest(){return {schemaVersion:1,id:'workload-triage',capabilityVersion:2,promptVersion:PROMPT_VERSION,purpose:'Assess current versus historical workload findings from bounded supplied facts.',
  invocation:{transport:'typescript-function',module:'@onionsoup/workload-triage',export:'triageWorkloads',signature:'(observation, options) => Promise<TriageRun>',
    ...
    checkpoint:'Admission, semantic rejections and terminal record. Persistence failure aborts; no automatic resume.'},contracts:{inputVersion:2,resultVersion:1,runVersion:1,inputSchema:z.toJSONSchema(WorkloadObservation),resultSchema:z.toJSONSchema(TriageResult)},limits:LIMITS,
...
export async function investigateWorkloads(raw:unknown,options:TriageOptions&{transport?:WorkloadTransport}){
  ...
  const signal=AbortSignal.any([AbortSignal.timeout(200000),...(options.signal?[options.signal]:[])]);
  try{
