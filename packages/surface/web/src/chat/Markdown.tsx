import { memo, useLayoutEffect, useMemo, useRef, type MouseEvent } from 'react';
...
marked.use({
  renderer: {
    code({ text, lang }: Tokens.Code) {
      ...
      return `<div data-component="markdown-code" data-lang="${escape(language)}" class="my-4 group overflow-hidden rounded-2xl border border-border/80 bg-[var(--surface-elevated)]">
...
export function renderMarkdown(text: string) {
  return DOMPurify.sanitize(marked.parse(text, { async: false }) as string, { ADD_ATTR: ['target', 'data-md-action', 'data-md-code-line-number', 'data-md-code-line', 'data-md-code-line-content', 'data-md-code-body', 'data-md-code-actions', 'data-component', 'data-markdown', 'data-lang'] });
}
...
export const Markdown = memo(function Markdown({ text, variant = 'assistant', className }: { text: string; variant?: 'assistant' | 'reasoning' | 'tool'; className?: string }) {
  ...
  useLayoutEffect(() => {
    ...
    // shiki loads only when a code block appears.
    if (element?.querySelector('[data-component="markdown-code"]')) void import('./highlight.ts').then(module => module.highlightCodeBlocks(element));
  }, [html]);
  ...
  return (
    <div className={`break-words w-full min-w-0 ${className ?? ''}`} onClick={onMarkdownClick}>
      <div ref={content} className={inner} data-markdown-content dangerouslySetInnerHTML={{ __html: html }} />
    </div>
  );
});
