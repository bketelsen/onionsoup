...
export const WorkloadSection = z.enum(['pods','jobs','replicasets','deployments']);
...
export function workloadCommand(raw: unknown, section: WorkloadSection) {
  ...
  return `test -x /usr/bin/k3s || exit 69; exec ${target.access==='sudo'?'/usr/bin/sudo -n ':''}/usr/bin/k3s kubectl --kubeconfig=/etc/rancher/k3s/k3s.yaml --context=default --server=https://127.0.0.1:6443 --request-timeout=10s --cache-dir=/dev/null get ${section} --all-namespaces --chunk-size=200 -o 'jsonpath={"v1\\n"}{range .items[*]}${templates[section]}{"\\n"}{end}'`;
}
...
const Base={id:ResourceId,namespaceId:ResourceId,createdAt:Time,deletingAt:Time,ownerId:ResourceId.nullable(),ownerKind:z.enum(['Job','ReplicaSet','Deployment','StatefulSet','DaemonSet','CronJob','Other']).nullable()};
...
export const WorkloadFact=z.union([PodFact,JobFact,ReplicaFact]); export type WorkloadFact=z.infer<typeof WorkloadFact>;
...
export function parseWorkloadProjection(section:WorkloadSection,stdout:string) {
  ...
  const rows=stdout==='v1\n'?[]:stdout.slice(3,-1).split('\n');if(rows.length>5000)throw new Error('Row limit');
  const lookup:Record<string,{namespace:string;name:string;kind:string}>={};
  const facts=rows.map(row=>{
    const f=row.split('|'),kind={pods:'Pod',jobs:'Job',replicasets:'ReplicaSet',deployments:'Deployment'}[section];
    if(f.length!==(section==='pods'?11:section==='jobs'?13:14))throw new Error('Invalid columns');
    ...
    const base={id,kind,namespaceId:resourceId('Namespace','',f[1]),createdAt:time(f[3]),deletingAt:time(f[4]),ownerId:f[6]?resourceId(f[5],f[1],f[6]):null,
      ownerKind:f[5]?(['Job','ReplicaSet','Deployment','StatefulSet','DaemonSet','CronJob'].includes(f[5])?f[5]:'Other'):null};
    if(section==='pods'){
...
export const WorkloadObservation=z.object({schemaVersion:z.literal(1),kind:z.literal('workload-observation'),runId:z.uuid(),assetId:KubernetesTarget.shape.assetId,
  ...
  if(new Set(ids).size!==ids.length||new Set(r.selected).size!==r.selected.length||new Set(r.queries.map(q=>q.section)).size!==4||
    r.selected.some(id=>!r.facts.some(f=>f.id===id&&f.kind==='Pod'))||r.status!=='running'&&(!r.finishedAt||r.status!==(n===4?'completed':n?'partial':'failed')||r.queries.some(q=>['pending','running'].includes(q.status)))||
...
export async function collectWorkloads(raw:unknown,options:{directory:string;signal?:AbortSignal;transport?:WorkloadTransport}){
  const target=KubernetesTarget.parse(raw);const record:WorkloadObservation={schemaVersion:1,kind:'workload-observation',runId:randomUUID(),assetId:target.assetId,targetHash:digest(target),commandVersion:'k3s-workloads-v1',startedAt:new Date().toISOString(),status:'running',
  ...
  const deadline=AbortSignal.timeout(85000),signal=AbortSignal.any([deadline,...(options.signal?[options.signal]:[])]);
  ...
  for(const q of record.queries){
    ...
    try{const response=await(options.transport??((t,s,a)=>runSshProcess(boundedSshArguments(t,workloadCommand(t,s)),a)))(target,q.section,signal);
      ...
      else{stage='invalid_output';const parsed=parseWorkloadProjection(q.section,response.stdout);facts.push(...parsed.facts);Object.assign(lookup,parsed.lookup);q.status='collected';}
  ...
  const n=record.queries.filter(q=>q.status==='collected').length;record.status=n===4?'completed':n?'partial':'failed';record.finishedAt=new Date().toISOString();
