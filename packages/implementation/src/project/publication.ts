...
export async function prepareProjectPublication(c:PublicationConfig,projectDirectory:string,rawTarget:unknown):Promise<ProjectBundle> {
  ...
  return locked(c,async()=>{
    ...
    const body=[w.job.proposal.result.outcome.text,'Request kind: feature. This is an owned-project proposal-to-draft trial. Human code-quality acceptance is not recorded.',
