  ...
  var connectHost = (options = {}) => {
    ...
    const subscribeWorkspace = async (query, listener) => {
      ...
      try {
        ...
        if (!disposed)
          post2({ ...envelope, type: "workspace-unsubscribe", id: nextId(ids), payload: { subscriptionId } });
        throw error;
      }
      return () => {
...
`;
  ...
  // src/panel.ts
  var REFRESH_MS = 2e4;
  var REGISTER_TABS = [["MAP", "Map"], ["WISDOM", "Wisdom"], ["decisions", "Decisions"], ["open-questions", "Open questions"], ["FAILURES", "Failures"]];
  var NOTE_LABELS = {
    "chat-decision": "Noted",
    "chat-action": "Did",
    "work-opened": "Opened work",
...
`;
  ...
  function button2(label, onClick, primary2 = false) {
    const node = element("button", primary2 ? "primary" : "", label);
  ...
  function when(at) {
    ...
    return Number.isNaN(date.getTime()) ? at : date.toLocaleString(void 0, { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });
  }
  ...
  async function post(path, payload) {
    ...
    const body = JSON.parse(result.body);
    if (result.status >= 400) throw new Error(body.error ?? `HTTP ${result.status}`);
    await host.toast({ kind: "success", message: body.output || "Recorded" }).catch(() => void 0);
    await refresh();
  }
  function renderWho(state, container) {
  ...
  function renderPending(state, container) {
    ...
    for (const pending of state.pending) {
      ...
      card.append(element("div", "", pending.title), element("div", "meta", `${pending.kind} \xB7 ${pending.id}`));
      ...
      row.append(button2(actions.approveLabel, () => void post("/decide", { action: actions.approve, id: pending.id, withDelete: withDelete?.checked }), true));
      if (actions.refuse) row.append(reason, button2(actions.refuseLabel, () => void post("/decide", { action: actions.refuse, id: pending.id, reason: reason.value })));
      card.append(row);
      container.append(card);
    }
  }
  function renderWork(state, container) {
    ...
    for (const item of state.work) {
      ...
      card.append(element("div", "", item.title), element("div", "status", `${item.status} \xB7 ${item.id}`));
      container.append(card);
    }
    for (const item of state.recent) {
      const line = element("div", "meta", `${item.status}: ${item.title}${item.url ? ` \xB7 ${item.url}` : ""}`);
  ...
  function renderNotes(state, container) {
    ...
    for (const note of state.notes) {
      ...
      card.append(element("div", "meta", `${NOTE_LABELS[note.kind] ?? note.kind}${note.outcome && note.kind === "chat-decision" ? ` (${note.outcome})` : ""} \xB7 ${when(note.at)}`));
      ...
      if (note.kind === "chat-decision" && !note.retracted) {
        const row = element("div", "row");
        row.append(button2("Retract \u2014 not a decision", () => void post("/retract", { owner: state.owner.id, note: note.note })));
        card.append(row);
      }
      container.append(card);
    }
  }
  function renderNotebook(state, container) {
    container.append(element("h2", "", "Notebook"));
    const tabs = element("div", "tabs");
    const body = element("div", "md");
    const show = () => {
      for (const child of tabs.children) child.classList.toggle("active", child.dataset.tab === activeTab);
      body.innerHTML = renderMarkdown(state.registers[activeTab] ?? "") || '<p class="empty">Empty.</p>';
    };
    for (const [key, label] of REGISTER_TABS) {
      const tab = button2(label, () => {
        activeTab = key;
        show();
      });
      tab.dataset.tab = key;
  ...
  function render(state) {
    ...
    root.append(desk);
    void host.setBadge(state.pending.length || null).catch(() => void 0);
  }
  ...
  host.onReady((context) => {
    ...
    directory = context.directory ?? void 0;
    void refresh();
