...
async function main() {
  ...
  if (multi && (issue as { issues: unknown[] }).issues.length !== 3) throw new Error('Workflow proof requires exactly three snapshots');
  ...
  await save('manifest.json', { schemaVersion: 1, consumer: 'codex-cli', model: EVALUATION_MODEL,
    provider, agentModel: EVALUATION_MODEL, recipe: multi ? 'prepared-multi-issue-readiness' : 'single-readiness',
    maxInvocations: multi ? 2 : 1, inputHash: multi ? readinessWorkflowInputHash(issue) : inputHash(IssueSnapshot.parse(issue)),
    expectedTools: multi ? ['discover_agents', 'assess_prepared_issues', 'inspect_workflow'] : ['discover_agents', 'assess_issue', 'inspect_run'],
    method: 'One external-consumer integration attempt; no automatic retry; not task accuracy.' });
  const answerSchema = multi ? { type: 'object', additionalProperties: false,
  ...
  const config = { command: process.execPath, args: ['--import', join(root, 'node_modules/tsx/dist/loader.mjs'), join(root, 'src/mcp-cli.ts')], env,
    required: true, tool_timeout_sec: multi ? 150 : 90, startup_timeout_sec: 20,
  ...
  const timeout = setTimeout(() => { timedOut = true; child.kill('SIGTERM'); }, multi ? 360000 : 240000);
