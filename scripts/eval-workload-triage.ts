...
const provider=providerName(process.argv[2]);if(!process.argv[2]||process.argv.length>3)throw new Error('Usage: eval-workload-triage.ts copilot|codex');
const directory=resolve('runs','workload-evaluation',randomUUID());await mkdir(directory,{recursive:true,mode:0o700});
const report:{schemaVersion:number;provider:string;model:string;directory:string;cases:unknown[]}={schemaVersion:1,provider,model:EVALUATION_MODEL,directory,cases:[]};
await atomicJson(join(directory,'evaluation.json'),report);
let failures=0;
for(const [name,expected] of cases){
  const run=await triageWorkloads(fixture(name),{directory:join(directory,name),provider,modelId:EVALUATION_MODEL,modelFactory:async()=>(await liveModel(EVALUATION_MODEL,provider)).model});
