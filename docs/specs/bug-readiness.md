...
This contract governs one issue assessment and the persisted evidence consumed by maintainers and downstream agents.
...
Request kind MUST remain separate from bug readiness and project acceptance. Historical v1 records retain their original labels. A valid classification may be non-bug; that is not a failed run.
