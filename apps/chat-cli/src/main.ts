...
const usage=`Usage: npm run chat -- --config MCP_CONFIG --provider copilot|codex [--session DIRECTORY --resume] [--target ID] [--message TEXT | --status]
...
`;
async function main(){
  const {values,positionals}=parseArgs({strict:true,options:{config:{type:'string'},provider:{type:'string'},session:{type:'string'},resume:{type:'boolean'},target:{type:'string'},message:{type:'string'},status:{type:'boolean'},help:{type:'boolean'}}});
  if(values.help){process.stdout.write(usage);return;}
  if(positionals.length||!values.config||!values.provider||values.resume&&!values.session||values.status&&values.message!==undefined)throw Error('ARGUMENTS');
  const provider=providerName(values.provider),configPath=resolve(values.config),config=HomelabMcpConfig.parse(await readJson(configPath));
  if(config.provider!==provider)throw Error('PROVIDER_MISMATCH');
  config.runsDirectory=resolve(dirname(configPath),config.runsDirectory);config.observations=config.observations.map(p=>resolve(dirname(configPath),p));
  let targetId=values.target;if(targetId&&!config.targets.some(t=>t.assetId===targetId))throw Error('TARGET_NOT_CONFIGURED');
  ...
  const profile=createHomelabChatProfile({bindingHash:hash({profile:HOMELAB_CHAT_VERSION,config}),call:async(name,args,signal)=>{
  ...
  try{
    ...
    const env=Object.fromEntries(Object.entries({PATH:process.env.PATH,HOME:process.env.HOME,SSH_AUTH_SOCK:process.env.SSH_AUTH_SOCK,ONIONSOUP_AUTH_PATH:process.env.ONIONSOUP_AUTH_PATH,
    ...
    for await(const raw of input){
      ...
      else if(line.startsWith('/target ')){const selected=line.slice(8).trim();if(config.targets.some(t=>t.assetId===selected)){targetId=selected;handle.session.memory=HomelabChatMemory.parse({...HomelabChatMemory.parse(handle.session.memory),selectedTarget:selected});await handle.save();process.stdout.write(`Selected ${selected}.\n`);}else process.stdout.write(`Configured targets: ${config.targets.map(t=>t.assetId).join(', ')}\n`);}
