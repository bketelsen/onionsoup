...
{:else}
  <table>
    <thead><tr><th>Status</th><th>Outcome</th><th>Capability</th><th>Created</th><th>Duration</th><th></th></tr></thead>
    <tbody>
      {#each jobs as job (job.jobId)}
        ...
        <tr>
          ...
          <td>{#if job.outcome}<span class={`outcome ${job.outcome.status}`}>{job.outcome.label}</span>{:else if job.error}<span class="outcome failed">{job.error.slice(0, 80)}</span>{/if}</td>
