...
try {
 ...
 else if(command==='propose'&&v.checkout&&v.commit&&v.output&&v.provider){const r=await proposeProject(resolve(v.checkout),v.commit,resolve(v.output),providerName(v.provider),{profile:v.profile as ProfileId|undefined,...(v['repository-profile']&&v.task?{repositoryProfile:RepositoryProfile.parse(await readJson(v['repository-profile'])),task:Task.parse(await readJson(v.task))}:{})});result={requestId:r.requestId,status:r.status,result:r.proposal?.result};}
