...
export async function repositoryAdapterHash() {
...
export function taskHarness(job:Job,raw:unknown) {
 if(job.schemaVersion!==2)throw new Error('Repository task required');validateTask(job.repositoryProfile,job.task,raw);const plan=VerificationPlan.parse(raw);
