...
export async function snapshot(checkout:string,commit:string,directory:string,binaryAssets=false) {
  ...
  for(const entry of entries) {
    ...
    const data=(await promisify(execFile)('/usr/bin/git',['-C',checkout,'cat-file','blob',m[2]],{encoding:'buffer',timeout:10000,maxBuffer:8000000,env:{PATH:'/usr/bin:/bin',GIT_CONFIG_NOSYSTEM:'1',GIT_CONFIG_GLOBAL:'/dev/null'}})).stdout;
