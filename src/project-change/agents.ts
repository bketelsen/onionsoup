...
export function validateProjectAgentRun(raw: unknown): ProjectAgentRun {
  ...
  if (!r || r.schemaVersion !== 2 || !FixtureAgentId.safeParse(r.agent).success || !z.uuid().safeParse(r.runId).success ||
      ...
      r.promptVersion !== prompts[r.agent].version || !Array.isArray(r.events) || !['running','completed','failed'].includes(r.status)) throw new Error('Invalid project agent record');
