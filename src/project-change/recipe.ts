...
export async function executeProject(checkout:string,proposalDirectory:string,options:{directory:string;runtime:Runtime;dependencies:Dependency;seedBundle?:unknown;provider:'copilot'|'codex';modelFactory?:typeof liveModel;verify?:typeof verifyProject;signal?:AbortSignal;persist?:typeof atomicJson}) {
  ...
  if(job.profileHash!==await profileHash(job.profile)||hash(await sourceFiles(checkout,job.baseCommit,job.allowedFiles))!==job.sourceHash)throw new Error('Frozen project inputs changed');
  const dir=resolve(options.directory);await mkdir(dir,{mode:0o700});const seeds=job.profile===GO_PROFILE?[]:seedsFrom(options.seedBundle);await atomicJson(join(dir,'seeds.json'),seeds);
  ...
  const verify=async(phase:'baseline'|'candidate')=>{
    signal.throwIfAborted();w[phase]=await(options.verify??(job.profile===GO_PROFILE?verifyGoProject:verifyProject))(phase==='baseline'?checkout:join(dir,'candidate'),phase==='baseline'?job.baseCommit:w.headCommit!,job,w.runtime,w.dependencies,seeds,
  ...
  try {
    ...
    if(patch.status==='needs_information')w.outcome='needs_information';
      ...
      w.headCommit=(await git(candidate,['-c','user.name=Onionsoup Agent','-c','user.email=agent@example.invalid','commit-tree',w.headTree,'-p',job.baseCommit,'-m',projectProfile(job.profile).title])).trim();
