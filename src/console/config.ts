...
export const ConsoleConfig=z.object({ schemaVersion:z.literal(1),stateDirectory:z.string().min(1),publicationConfig:z.string().min(1).optional(),fixtureRoots:z.array(z.string().min(1)).max(10).default([]),
...
export async function loadConsoleConfig(file:string) {
  ...
  return { ...c,stateDirectory:path(c.stateDirectory),publicationConfig:c.publicationConfig?path(c.publicationConfig):undefined,fixtureRoots:c.fixtureRoots.map(path),jobs:c.jobs.map(j=>({ ...j,deliveryConfig:path(j.deliveryConfig),
