import { Query } from '../change-proposal/record.ts';
import { z } from 'zod';
...
export const OperatorRequest=z.discriminatedUnion('action',[
  ...
  z.object({ ...base,action:z.literal('propose'),parentOperationId:z.uuid(),packetId:z.uuid(),packetHash:digest,query:Query.optional() }).strict(),
...
export const OperatorRecord=z.object({ schemaVersion:z.literal(1),kind:z.literal('operator-action'),workflowId:z.uuid(),
  ...
  result:z.discriminatedUnion('type',[
    ...
    z.object({ type:z.literal('proposal'),workflowId:z.uuid(),status:z.enum(['running','completed','failed']) }).strict(),
  ...
  if(r.result && ((r.request.action==='run_now'||r.request.action==='retry') && r.result.type!=='delivery' ||
    ...
    r.request.action==='propose'&&r.result.type!=='proposal' || r.request.action==='investigate'&&!['packet','skipped'].includes(r.result.type))) fail('Action result mismatch');
});
