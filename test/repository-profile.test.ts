...
async function setup(t:any){
 ...
 const parent=await proposeProject(repo,base,dir,{repositoryProfile:profile,task:def,models:prepareModel});assert.equal(parent.status,'completed');
...
test('real reusable Go adapter rejects absent feature while preserving standard checks',{skip:!process.env.ONIONSOUP_GO_CHECKOUT},async t=>{
 ...
 await proposeProject(checkout,task.baseCommit,dir,{repositoryProfile:profile,task,models:prepareModel});const job=await acceptProject(checkout,dir,mapping,'Scripted integration preflight.',{runtime:rt,dependencies:deps,verificationPlan:plan});
